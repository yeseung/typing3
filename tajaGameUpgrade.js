var taja = ["가위", "가을", "강아지", "개구리", "거울", "겨울", "고양이", "구름",
...
function drawTaja() {
    ...
    var drawInterval = setInterval(function () {
        setConstructor.push(new TajaConstructor({
...
// 느리게 내려오기
function easyDown(tajaArg) { // 받아온 this.tajaDiv를 이용합니다.
    var topVal = 0;
    setInterval(function () {
        ...
        if (topVal + tajaArg.offsetHeight >= tajaContents.offsetHeight) {
            if (tajaContents.contains(tajaArg)) {
                ...
                if (setConstructor.length === taja.length) { // 화면에 단어가 다 뿌려진 이후
...
function zigzagDown(tajaArg) {
    ...
    setInterval(function () {
        ...
        } else if (tajaArg.offsetLeft + tajaArg.offsetWidth >= tajaContents.offsetWidth) {
            tajaArg.style.left = (tajaContents.offsetWidth - (tajaArg.offsetWidth + 50)) + "px";
            zigzagVal = -zigzagVal;
            count = 0;
